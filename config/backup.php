...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                /*
                 * Database-only backups — no source files included.
                 */
                'include' => [],
        ...
        'destination' => [
             ...
             * Always writes to the local 'backups' disk.
             * When BACKUP_SERVER_HOST is set (show-day live server), also writes to
             * the 'backup-server' SFTP disk so the backup server receives every snapshot.
             */
            'disks' => array_filter([
                'backups',
                env('BACKUP_SERVER_HOST') ? 'backup-server' : null,
            ]),
    ...
    'notifications' => [
        ...
        'mail' => [
            'to' => env('BACKUP_NOTIFICATION_EMAIL', env('MAIL_FROM_ADDRESS', 'admin@villageshow.org')),
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
            /*
             * Keep every backup taken within the last day.
             * In show mode (every 5 min) this preserves all intraday snapshots,
             * giving the operator a fine-grained restore point if needed.
             */
            ...
            /*
             * 2 GB ceiling per destination disk — well above what a show DB needs.
             */
