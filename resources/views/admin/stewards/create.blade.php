<x-layouts::app :title="__('Add Steward')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 p-4">
        <div class="flex items-center gap-2">
            <flux:button :href="route('admin.stewards.index')" variant="ghost" icon="arrow-left" size="sm" wire:navigate>
                Stewards
            </flux:button>
            <flux:heading size="xl">Add Steward</flux:heading>
        </div>

        <form method="POST" action="{{ route('admin.stewards.store') }}" class="max-w-lg space-y-4">
            @csrf
            ...
            <p class="text-sm text-zinc-500">A temporary password will be generated automatically. The steward will need to reset it on first login.</p>

            <div class="flex gap-2">
                <flux:button type="submit" variant="primary">Add Steward</flux:button>
